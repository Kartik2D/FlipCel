...
import { magicMorphUiStore, StoreController } from "../../state";
...
export class InkwellMagicMorphPopup extends LitElement {
  ...
  private ui = new StoreController(this, magicMorphUiStore);

  private _outsideClickHandler = (e: PointerEvent) => {
    if (!this.open) return;
    const path = e.composedPath();
    if (!path.includes(this)) {
      magicMorphUiStore.update((s) => ({ ...s, popupOpen: false }));
    }
  };
  ...
  private onApply() {
    this.dispatchEvent(
      new CustomEvent("magic-morph-apply", {
        bubbles: true,
        composed: true,
      }),
    );
